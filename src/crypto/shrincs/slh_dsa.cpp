...
namespace SLH_DSA
{
    void slh_dsa_digest_message(const unsigned char* r, const unsigned char* pk_seed, const unsigned char* sl_root, const unsigned char* message, uint32_t m_len, unsigned char* out_digest, uint64_t* out_tree, uint32_t* out_leaf)
    {
        ...
        unsigned char tree_index[8] = {}, leaf_index[8] = {};

        memcpy(tree_index + 8 - tree_index_size, tree_index_digest, tree_index_size);
        *out_tree = ReadBE64(tree_index) & ((UINT64_C(1) << SPHX_TREE_INDEX_BITS) - 1);

        memcpy(leaf_index + 8 - leaf_index_size, leaf_index_digest, leaf_index_size);
        *out_leaf = ReadBE64(leaf_index) & ((UINT32_C(1) << SPHX_XMSS_HEIGHT) - 1);
    }
