...
enum : uint32_t {
    ...
    // ELEMENTS:
    // Enable OP_SHRINCS / OP_SHRINCSADD.
    // Without this flag the two opcodes keep their previous OP_NOP4 / OP_NOP5 semantics
    //
    SCRIPT_VERIFY_SHRINCS = (1U << 24),
...
protected:
    ...
    virtual bool VerifySHRINCSSignature(const std::vector<unsigned char>& sig, const std::vector<unsigned char>& pubkey, const uint256& sighash) const;
