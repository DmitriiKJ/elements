...
def make_signblockscript(num_nodes, required_signers, keys):
    ...
    script += "29ab27ec700ca13c532563d0a0708a0c4bdcaf7f38409293a679e189f517b8bc"
    script += "29ab27ec700ca13c532563d0a0708a0c4bdcaf7f38409293a679e189f517b8ba"
    script += "29ab27ec700ca13c532563d0a0708a0c4bdcaf7f38409293a679e189f517b8ba"
...
class BlockSignTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        self.num_nodes = 4 # 3 нод-майнерів, 1 нода-спостерігач
        self.num_keys = 3  # 3 ключі для підпису
        self.required_signers = 2 # 2-з-3 мультипідпис
        ...
        self.witnessScript = signblockscript

        my_pq_key = "29ab27ec700ca13c532563d0a0708a0c4bdcaf7f38409293a679e189f517b8ba967fef422d4dc6c9bc7ca0cac3e1185b95e85f3d7035d4a0cf780eb661bee1b9c522c648c2ec264290a5e2daf5a3516d46cfcaae59053efb09a25e99affb3d86"
        
        self.extra_args = [[
            "-signblockscript={}".format(signblockscript),
            f"-pqminerkey={my_pq_key}",
            "-con_max_block_sig_size=100000",
            "-anyonecanspendaremine=1",
        ]] * self.num_nodes

    def run_test(self):
        self.nodes[0].importprivkey(self.wifs[0])

        self.log.info("Starting Minimal SHRINCS PoC Test...")

        for i in range(3): 
            self.log.info(f"Mining block {i+1}...")

            block = self.nodes[0].getnewblockhex()

            sigs = []
            for i in range(self.required_signers):
                sigs = sigs + self.nodes[i].signblock(block, self.witnessScript, True)

            # Uncomment to make test fail
            # bad_sig_hex = sigs[0]["sig"][:-4] + "0000"
            # sigs[0]["sig"] = bad_sig_hex
            
            result = self.nodes[0].combineblocksigs(block, sigs, self.witnessScript, True)
            
            submit_result = self.nodes[0].submitblock(result["hex"])

            self.log.info(f"Submit result: {submit_result}")

            current_height = self.nodes[0].getblockcount()
            self.log.info(f"Current blockchain height: {current_height}")
            
            if submit_result is not None:
                raise AssertionError(f"Block was REJECTED by node! Reason: {submit_result}")

            self.sync_all()
    
        self.log.info("SUCCESS! SHRINCS signatures were created and accepted by the network!")
